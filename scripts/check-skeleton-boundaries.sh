...
unexpected_mutations="$(printf '%s\n' "$mutation_routes" | grep --invert-match --extended-regexp \
  '^(apps/api/src/agentbox_api/auth\.py:.*@router\.post\("/(login|logout|reauthenticate)"|apps/api/src/agentbox_api/codex\.py:.*@router\.post\("/(remote/start|remote/stop|pair-codes)"|apps/api/src/agentbox_api/claude\.py:.*@router\.post\("/sessions/\{project_id\}/(start|stop)"|apps/api/src/agentbox_api/projects\.py:.*@router\.post\(|apps/api/src/agentbox_api/workspaces\.py:.*@(router|project_workspaces_router)\.post\()' || true)"
