...
Owner 说“继续项目”时，重新验证 live state，读取 `NEXT_ACTION`，只执行唯一仍获授权的动作；Review 必须使用 read-only reviewer subagents。发现超出授权范围的架构或 Slice 需求即 STOP/BLOCKED。等待 exact-head CI terminal，更新 branch-local snapshot，并停在下一个 Owner gate。不得自动 Ready、merge、开始下一个 Slice、改写 Accepted architecture 或处理真实 Secret。
...
`feature branch → Draft PR → CI → Architecture/Security Review → Owner explicit approval → Ready → Squash Merge → exact read-back`。禁止 force push、`--admin`、自动 Ready/merge/next Slice。只在授权 branch 上写入，不直接写 main。
...
`CURRENT_STATE` 是 verified snapshot，不是 trust root；每次 task 结束在当前 branch 更新。merge SHA 只能通过 merge read-back 得到；发现 stale state 先报告再在适当 branch 更新。

## Owner Gate

只有人类 Owner 可以发送 `Owner Merge Authorization: GRANTED`，且必须绑定 exact PR number、head SHA、base SHA；head/base 变化即失效。未收到该声明不得 Ready 或 merge。
