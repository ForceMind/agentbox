...
def test_detach_timeout_closes_but_does_not_release_writer_slot() -> None:
    clock = Clock()
    fence = _fence(clock)
    fence.begin(attachment_id="att_1", generation=1, lease_number=2)
    fence.commit_admission()
    ...
    clock.value = 101
    closed = fence.tick()
    assert closed.state is LeaseCleanupState.CLOSED
    assert closed.writer_slot_reserved
    assert fence.request_detach().state is LeaseCleanupState.CLOSED
