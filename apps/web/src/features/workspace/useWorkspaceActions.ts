...
export function useWorkspaceActions() {
  ...
  const start = useCallback(
    (projectId: string) =>
      run('start', () =>
        api.post(
          `/api/v1/projects/${encodeURIComponent(projectId)}/workspaces/claude/start`,
          {
            body: {},
            csrfToken: auth?.csrf_token,
            validate: parseWorkspaceStartResponse,
          },
        ),
      ),
    [api, auth, run],
  ...
  const connect = useCallback(
    (workspaceId: string) =>
      run<WorkspaceAttachmentTicketResponse>('connect', () =>
        api.post(
          `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/attachments`,
          {
            body: { mode: 'writer' },
            csrfToken: auth?.csrf_token,
            validate: parseWorkspaceAttachmentTicketResponse,
          },
        ),
      ),
    [api, auth, run],
  ...
  const reconnect = useCallback(
    (workspaceId: string) =>
      run<WorkspaceAttachmentTicketResponse>('reconnect', () =>
        api.post(
      ...
      run('detach', () =>
        api.post(
          `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/detach`,
          {
            ...
            csrfToken: auth?.csrf_token,
            validate: parseWorkspaceDetachResponse,
          },
        ),
      ),
    [api, auth, run],
  ...
  const stop = useCallback(
    (workspaceId: string, generation: string) =>
      run('stop', () =>
        api.post(`/api/v1/workspaces/${encodeURIComponent(workspaceId)}/stop`, {
          body: { generation },
          csrfToken: auth?.csrf_token,
          validate: parseWorkspaceStopResponse,
        }),
      ),
    [api, auth, run],
  )
