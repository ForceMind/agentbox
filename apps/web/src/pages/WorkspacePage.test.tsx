...
describe('WorkspacePage security UI skeleton', () => {
  it('starts unadmitted and keeps actions disabled until transport exists', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Interactive Workspace' })).toBeInTheDocument()
    expect(screen.getByText('检查中')).toBeInTheDocument()
    expect(screen.getByText('NOT ADMITTED')).toBeInTheDocument()
    expect(screen.getByText(/不会伪造 workspace、ticket 或运行状态/)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Start / Connect' })).toBeDisabled()
    expect(screen.getByRole('button', { name: /Stop workspace/ })).toBeDisabled()
  })
  ...
  it('shows the ADMITTED marker only for connected state', () => {
    const { rerender } = render(<WorkspacePage initialState={state('connecting')} />)
    expect(screen.getByText('NOT ADMITTED')).toBeInTheDocument()
    expect(screen.queryByText('Runtime ADMITTED 已确认')).not.toBeInTheDocument()
  ...
  it('keeps sensitive terminal data out of browser storage and exposes no transcript action', () => {
    ...
    expect(screen.queryByRole('button', { name: /transcript|下载/i })).not.toBeInTheDocument()
