...
| T-114 | T/E | Credential Runtime ownership is moved after creation | attacker can issue an ordinary update or direct SQL | an existing opaque Secret reference changes Store meaning | no owner-update method; `BEFORE UPDATE` immutability trigger; later movement requires separately authorized migration | ORM/update absence scan and direct-SQL trigger tests | database owner with power to replace schema/triggers is outside workflow-integrity protection |
...
| T-117 | T | Canonical digest ambiguity or cross-purpose substitution changes the reviewed plan | inconsistent serialization, locale, null, timestamp, or field ordering exists | approval binds a different Runtime/Provider/Credential/postcondition | domain-separated SHA-256 over RFC 8785 canonical UTF-8 object with exact field set/representations and constant-time compare | golden vectors for nulls, timestamps, Unicode, integers, order, and purpose/domain changes | digest is not a signature or protection from a compromised Control Plane |
| T-118 | S/E | Challenge is rebound to another Session or newer authentication epoch | same Admin logs in/re-authenticates elsewhere or Session state changes | one context consumes another context's approval | exact Admin/Session FK, durable `auth_epoch` and `recent_authenticated_at`, same-Session checks, no raw token copy | second-login, second-Session, newer-reauth, and Session-pruning tests | theft of the exact still-valid browser Session remains within normal Session risk |
...
| T-120 | T/R | Cancellation races consumption or two consumers both succeed | concurrent requests target one issued challenge | duplicate attempts or contradictory terminal evidence | conditional `ISSUED` transition and attempt insertion in one `BEGIN IMMEDIATE`; one active attempt partial index | consume/consume and consume/cancel concurrency tests across commit failures | SQLite host/database unavailability can deny service but cannot create two committed winners |
...
| T-124 | D/I | Challenges grow without bound or are pruned too early | repeated issue/terminal transitions or maintenance errors | disk exhaustion or loss of replay/reconciliation evidence | five-minute eligibility; 30-day ordinary terminal retention; 90-day `NEEDS_ATTENTION` retention; batch 100 ordered cleanup; active rows never pruned; FK `RESTRICT` | volume, cutoff equality, batch/order, active-row, and contention tests | terminal Control Plane evidence ages out after its fixed investigation window while independent Audit/Runtime retention remains authoritative for its own scope |
| T-125 | T/R | Challenge is pruned while provisioning orchestration still needs it | independent retention jobs race or run out of order | recovery loses approval correlation | delete terminal attempts before challenges; attempt/challenge FK `RESTRICT`; only terminal retention-eligible rows | pruning race and direct-delete FK tests | prolonged unresolved attempts retain metadata longer than the ordinary window |
...
| T-128 | T/D | Crash occurs between approval consumption and durable orchestration creation | process/database fails during consume | consumed approval has no recoverable intent, or an intent exists without approval | challenge transition, `psa_*`, `psi_*`, and both Audit records share one `BEGIN IMMEDIATE`; rollback is all-or-none | fault injection before/after every statement, flush, commit, and busy timeout | committed `AUTHORIZED` work may require later reconciliation but is never silently replayed |
| T-129 | T/I/D | Unsafe `0005` upgrade/downgrade or Secret-shaped generic metadata loses authority or leaks sensitive values | migration sees legacy/new data or a caller reaches generic Audit/JSON | ownership/approval evidence is discarded, global uniqueness is recreated unsafely, or Secret derivatives persist | exact preflights and rebuild inventory; fail-closed downgrade predicates; typed columns; per-action metadata allowlists; no arbitrary JSON or Secret-derived values | fresh/upgrade/downgrade fault matrix, schema diff, forbidden-field reflection, and SQLite/WAL/SHM/Audit/log canary scans | verified backup and separate operator recovery are required after feature use; no automatic destructive downgrade exists |
