...
| T-124 | D/I | Authority rows grow without bound, are pruned too early, or inconsistent batch limits exceed the bound | repeated issue/terminal transitions or maintenance category fan-out | disk exhaustion or loss of replay/reconciliation evidence | five-minute eligibility; safe terminal 30-day eligibility; unresolved `NEEDS_ATTENTION` never ordinary-pruned and blocks one Credential; one deterministic union limits all expiry/prune/Session categories to 100 total | volume, cutoff equality, global total/category/order, protected-row and contention tests | unresolved evidence persists until a separately authorized lifecycle action; one row per Credential bounds repeated retries |
...
| T-128 | T/D | Crash occurs between approval consumption and durable orchestration creation | process/database fails during consume | consumed approval has no recoverable attempt, or an attempt substitutes a different intent | pre-generated digest-bound `psi_*`; matching Attempt inserted before Challenge consume; triggers require exact tuple; one `BEGIN IMMEDIATE` includes transitions and Audit | direct-SQL mismatch plus fault injection before/after every statement, version, flush, commit, and busy timeout | committed `AUTHORIZED` work may require later reconciliation but is never silently replayed |
| T-129 | T/I/D | Unsafe `0005` upgrade/downgrade, Alembic transaction mismatch, or forbidden Audit metadata loses authority/leaks values | FK mode changes inside a transaction, schema/version commit separately, or sanitizer rejects an authority Audit | invalid schema/FKs, partial version state, transaction rollback/DoS, or Secret derivatives persist | exact `AUTOCOMMIT` connection/FK-off/one `BEGIN IMMEDIATE` runner includes Alembic version row; post-commit restore gate; fail-closed downgrade; sanitizer-compatible exact keys and typed columns | temporary Alembic rollback/commit prototype, migration fault matrix, schema/version diff, sanitizer tests, and SQLite/WAL/SHM/Audit/log canaries | post-commit verification failure requires verified-backup restoration; full Control Plane compromise remains residual |
...
| T-131 | T/E | Direct SQL mutates Challenge/Attempt authority or inserts mismatched cross-table tuples | attacker bypasses repository methods | approval is moved, replayed, regressed, reconciled early, or bound to another `psi_*` | composite Challenge/intent and Admin/Session FKs; immutable-tuple, legal-transition, consistency, consumed-attempt and insert-match triggers; exact enums | direct-SQL mutation/transition/mismatch corpus and transaction fault injection | full Control Plane DB/application compromise can replace enforcement and forge authority |
...
| T-134 | T/R | Lost cancellation acknowledgement is reported as cancelled while Runtime is already consuming | Control Plane requested cancellation after Runtime staging | encrypted orphan is blindly retried or falsely declared absent | `RUNTIME_STAGED -> CANCEL_PENDING`; only exact Runtime confirmation reaches `CANCELLED`; consuming/verified observation wins; contradiction becomes `NEEDS_ATTENTION`; same `psi_*` status only | lost response, consuming race, verified race, contradiction, no-new-intent tests | Runtime compromise can forge its status within the accepted same-UID residual risk |
...
| T-136 | T/D | `NEEDS_ATTENTION` pruning enables blind reprovision | time-based cleanup treats unresolved state as terminal disposable data | a second attempt targets an unresolved Runtime orphan | include `NEEDS_ATTENTION` in unresolved partial index and Challenge insert guard; no ordinary delete/resolve; separate lifecycle authorization required | cutoff/maintenance/direct-delete/admission tests after arbitrary time passage | unresolved metadata remains indefinitely until explicit review, bounded to one per Credential |
