...
| R11 software integration | 进行中 | 全链路 controller、故障注入、E2E、artifact、操作文档与全页面双语 | R4–R10；API public anchor、同一Runtime peer/pidfd与durable epoch 已在 rc6 组合；redraw 已由 `f37f92d9...` 20/20 exact-head CI 验证。当前未提交 `WAWRuntimeApplication` slice 以 distinct one-shot activated-socket/key/executor ownership、shared start/close tasks、同一 control/legacy gate、private partial builders、unforgeable one-use composition binding、retryable construction cleanup owner及六项 shutdown evidence闭合 Runtime top-level ownership；provider仅在stream/control/lifecycle/legacy全clean后关闭。独立 Sol/xhigh PASS且无P0/P1/P2，Linux-target mypy 260 sources；不启用 production main、real key/provider 或 host。之后组合 API singleton 与 browser/API/Runtime。无持久 payload/key，覆盖 audit/commit/queue/exit/revoke/cancel 矩阵、CI 和独立审查 |
